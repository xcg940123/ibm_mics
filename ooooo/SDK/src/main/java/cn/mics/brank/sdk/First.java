...
public void checkapplication(Channel channel,String contractID,String medicareID) {
	try {
			Collection<Orderer> orderers = channel.getOrderers();
			Collection<Peer> channelPeers = channel.getPeers();
			CheckChaincode CheckChaincode = new CheckChaincode(contractID, medicareID,testTxID, channelPeers, orderers, channel, client,sampleOrg,successful, failed, TESTUSER_1_NAME, chaincodeID);
			Out.out("Query folks!4");
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
