...
import cn.mics.brank.fabric.entity.Result;
import cn.mics.brank.fabric.entity.Error;
...
public class Util {
    ...
    public static String getErrorMessage(String field) {
    	...
    	error.setCode(PropertiesUtil.getInstance().getErrId(PropertiesUtil.ArgMissInvalidId));
		error.setStatement(PropertiesUtil.getInstance().getErrMsg(PropertiesUtil.ArgMissInvalidMsg, new String[]{field}));
		
		errors.add(error);
		
		return JacksonUtil.toJSon(Result.constructFailDTO(errors));
	}
