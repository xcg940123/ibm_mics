...
type  Contract struct{
    ...
    contractState string `json："phoneNumber2"`
}
...
fmt.Println("########### example_cc save ###########")
	var   medicareID string   //人名加相关数据名称
	var medicareValue  string
	var err error
     if len(args) != 3 {
		return shim.Error("Incorrect number of arguments. Expecting 4")
	}
    ...
    medicareID = args[1]
    medicareValue = args[2]
	fmt.Printf("medicareID=%s,medicareValue = %s\n",medicareID,medicareValue)
